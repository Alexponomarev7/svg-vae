...
def _convert_to_path(g):
  ...
  path = sfd_to_path_list(g)
  path = add_missing_cmds(path, remove_zs=False)
  path = normalize_based_on_viewbox(
...
def _create_example(pathuni):
  ...
  final['rendered'] = per_step_render(path, absolute=True)

  # make path relative
  path = make_relative(path)
  # convert to vector
  vector = path_to_vector(path, categorical=True)
  # make simple vector
  ...
  final['class'] = int(map_uni_to_alphanum(uni))
  final['binary_fp'] = str(binary_fp)

  # append eos
  vector = append_eos(vector.tolist(), True, 10)
